<template>
    <div class="p-10">
        <div class="text-2xl mb-5">買主編集</div>
...
export default {
    ...
    methods: {
        async getData(){
            try{
                const res = await this.$axios.$get('/admin/buyers/'+ this.$route.params.id)
                this.form = res.data
        ...
        async update(){
            try{
                const res = await this.$axios.$post('/admin/buyers/update', this.form)

                this.$router.push({
                    path: '/admin/buyers',
                    query :{ success_message: '買主の編集が完了いたしました'},
                })
